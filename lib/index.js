...
var stderrToIgnore = 'Picked up JAVA_TOOL_OPTIONS: -Xmx384m  -Djava.rmi.server.useCodebaseOnly=true';
  ...
  async.waterfall([
    ...
    function launchTika(cb) {
      shellExec('java -jar ' + config.tika_path + ' ' + path, {maxBuffer: 8 * 1024 * 1024}, function(err, stdout, stderr) {
        // Java put its config on stderr. So if tika does not return any data, there is a stderr and no stdout so we suppose it's an error
        if(stderr.trim() === stderrToIgnore) {
          stderr = "";
        }
        if(err) {
